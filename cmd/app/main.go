...
func main() {
	log.Println("🚀 Starting Articles gRPC Service...")
	...
	docService := services.NewArticleService(repo)
	...
	pb.RegisterArticleServiceServer(grpcServer, grpcserver.NewArticleServer(docService))
...
func loadDatabase() (*sql.DB, *postgres.ArticleRepository) {
	...
	return db, postgres.NewArticleRepository(db)
}
